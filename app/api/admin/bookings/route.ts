...
function auth(request:Request){const s=adminSessionFromRequest(request);return s&&roleCan(s.role,"orders")?s:null}
...
export async function GET(request:Request){
  if(!auth(request))return Response.json({message:"غير مصرح لعرض الطلبات"},{status:403});
