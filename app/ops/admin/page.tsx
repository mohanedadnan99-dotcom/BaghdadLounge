...
type LoungeStatus={loungeName:string;currentSupervisor:string;username:string;role:string;shiftName:string;openedAt:string};
...
export default function OpsAdminPage(){
  ...
  return <main dir="rtl" style={{minHeight:"100vh",background:"#07111f",color:"#f8fafc",padding:16,fontFamily:"Arial,sans-serif"}}>
    <div style={{maxWidth:1280,margin:"0 auto"}}>
      <header style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:14,flexWrap:"wrap",marginBottom:22}}>
        <div><div style={{color:"#c8a66a",fontWeight:800}}>BAGHDAD LOUNGE OPERATIONS</div><h1 style={{margin:"6px 0",fontSize:28}}>لوحة إدارة الصالات</h1><div style={{color:"#94a3b8"}}>إدارة لاونج بغداد وعراق لاونج والشفتات والموظفين</div></div>
      ...
      {view==="dashboard"?<>
        <section style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))",gap:14,marginBottom:18}}>
          {loungeNames.map(name=>{
            ...
            return <div key={name} style={{...card,minHeight:145}}>
              <div style={{color:"#c8a66a",fontWeight:900,fontSize:18}}>{name}</div>
              <div style={{marginTop:18,color:"#94a3b8",fontSize:13}}>مسؤول الشفت الحالي</div>
              <div style={{fontSize:22,fontWeight:900,marginTop:5}}>{status?.currentSupervisor||"لا يوجد شفت مفتوح"}</div>
              <div style={{marginTop:8,color:"#cbd5e1",fontSize:14}}>{status?.shiftName?`${status.shiftName} · ${roleLabels[status.role]||status.role}`:"بانتظار فتح الشفت"}</div>
            </div>
          })}
        </section>
        ...
        <form onSubmit={addEmployee} style={card}>
          ...
          <button style={{...button(true),width:"100%"}}>إضافة الموظف</button>
        </form>

        <section style={card}>
          <h2 style={{marginTop:0}}>الموظفين والصلاحيات</h2>
          <div style={{display:"grid",gap:12}}>{employees.length?employees.map(employee=><div key={employee.id} style={{border:"1px solid #253247",borderRadius:14,padding:14,display:"grid",gap:10}}>
            <div style={{display:"flex",justifyContent:"space-between",gap:10,alignItems:"center",flexWrap:"wrap"}}><div><div style={{fontWeight:900,fontSize:17}}>{employee.name}</div><div style={{color:"#94a3b8",marginTop:4}}>@{employee.username}</div></div><button type="button" onClick={()=>patchEmployee(employee,{active:!employee.active})} style={button(employee.active)}>{employee.active?"نشط":"موقوف"}</button></div>
          ...
          </div>):<div style={{color:"#94a3b8"}}>ماكو موظفين مضافين بعد</div>}</div>
        </section>
      </div>}
    </div>
  </main>
...
function Stat({title,value}:{title:string;value:number|string}){return <div style={card}><div style={{color:"#94a3b8",fontSize:14}}>{title}</div><div style={{fontSize:27,fontWeight:900,marginTop:7}}>{String(value)}</div></div>}
