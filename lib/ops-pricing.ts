...
async function createOpsPricingTables(){
  ...
  await db`CREATE TABLE IF NOT EXISTS ops_pricing_rules(id BIGSERIAL PRIMARY KEY,rule_type TEXT NOT NULL,key_value TEXT NOT NULL,label TEXT NOT NULL DEFAULT '',price_iqd BIGINT NOT NULL,payment_type TEXT NOT NULL DEFAULT 'cash',active BOOLEAN NOT NULL DEFAULT TRUE,priority INT NOT NULL DEFAULT 100,valid_from TIMESTAMPTZ,valid_to TIMESTAMPTZ,metadata JSONB NOT NULL DEFAULT '{}'::jsonb,created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW())`;await db`CREATE INDEX IF NOT EXISTS ops_pricing_rules_lookup_idx ON ops_pricing_rules(rule_type,key_value,active,priority)`;
  await db`CREATE TABLE IF NOT EXISTS ops_pricing_audit(id BIGSERIAL PRIMARY KEY,actor TEXT NOT NULL DEFAULT '',action TEXT NOT NULL,entity_type TEXT NOT NULL,entity_id TEXT NOT NULL DEFAULT '',before_json JSONB,after_json JSONB,created_at TIMESTAMPTZ NOT NULL DEFAULT NOW())`;
...
export async function savePricingRule(input:any,actor="system"){await ensureOpsPricing();const db=sql();const ruleType=String(input.ruleType||'');if(!['lounge','shift','category','promotion','airline','special'].includes(ruleType))throw new Error('نوع قاعدة السعر غير صحيح');const key=String(input.keyValue||'').trim();if(!key)throw new Error('قيمة القاعدة مطلوبة');const payment=payments.includes(String(input.paymentType))?String(input.paymentType):'cash';if(input.id){const before=(await db`SELECT * FROM ops_pricing_rules WHERE id=${Number(input.id)}`)[0];const rows=await db`UPDATE ops_pricing_rules SET rule_type=${ruleType},key_value=${key},label=${String(input.label||'')},price_iqd=${Math.max(0,Number(input.priceIqd||0))},payment_type=${payment},active=${input.active!==false},priority=${Number(input.priority||100)},valid_from=${input.validFrom||null},valid_to=${input.validTo||null},metadata=${JSON.stringify(input.metadata||{})}::jsonb,updated_at=NOW() WHERE id=${Number(input.id)} RETURNING *`;await db`INSERT INTO ops_pricing_audit(actor,action,entity_type,entity_id,before_json,after_json) VALUES(${actor},'update','rule',${String(input.id)},${JSON.stringify(before||{})}::jsonb,${JSON.stringify(rows[0]||{})}::jsonb)`;return rows[0]}const rows=await db`INSERT INTO ops_pricing_rules(rule_type,key_value,label,price_iqd,payment_type,active,priority,valid_from,valid_to,metadata) VALUES(${ruleType},${key},${String(input.label||'')},${Math.max(0,Number(input.priceIqd||0))},${payment},${input.active!==false},${Number(input.priority||100)},${input.validFrom||null},${input.validTo||null},${JSON.stringify(input.metadata||{})}::jsonb) RETURNING *`;await db`INSERT INTO ops_pricing_audit(actor,action,entity_type,entity_id,after_json) VALUES(${actor},'create','rule',${String((rows[0] as any).id)},${JSON.stringify(rows[0])}::jsonb)`;return rows[0]}
