...
import { ensureCaptainOrdersTable } from "./captain-orders-db";
import { autoArchiveOldOrders, ensureAdminOpsFields, updateOrderOps } from "./admin-ops-db";
...
export type AdminBooking = {
  ...
  priority:"normal"|"important"|"urgent"; internal_note:string; archived_at:string|null; updated_at:string;
};
...
export async function adminOverview(){
  ...
  const stats=await db`
    WITH all_orders AS (
      SELECT created_at,status,passengers,total_iqd,archived_at FROM lounge_bookings
      UNION ALL
      SELECT created_at,status,passengers,0 AS total_iqd,archived_at FROM captain_lounge_orders
    )
    SELECT
      COUNT(*) FILTER (WHERE archived_at IS NULL AND (created_at AT TIME ZONE 'Asia/Baghdad')::date=(NOW() AT TIME ZONE 'Asia/Baghdad')::date)::int AS bookings_today,
      COUNT(*) FILTER (WHERE archived_at IS NULL AND status='new')::int AS new_bookings,
      COUNT(*) FILTER (WHERE archived_at IS NULL AND status='completed' AND (created_at AT TIME ZONE 'Asia/Baghdad')::date=(NOW() AT TIME ZONE 'Asia/Baghdad')::date)::int AS completed_today,
      COALESCE(SUM(passengers) FILTER (WHERE archived_at IS NULL AND (created_at AT TIME ZONE 'Asia/Baghdad')::date=(NOW() AT TIME ZONE 'Asia/Baghdad')::date),0)::int AS passengers_today,
      COALESCE(SUM(total_iqd) FILTER (WHERE archived_at IS NULL AND (created_at AT TIME ZONE 'Asia/Baghdad')::date=(NOW() AT TIME ZONE 'Asia/Baghdad')::date),0)::bigint AS revenue_today,
  ...
  const captainList=await db`SELECT id,username,name,company,phone,active FROM captain_accounts ORDER BY name`;
  ...
  const recent=await db`
    SELECT * FROM (
      SELECT id::bigint AS sort_id,id::int AS id,reference,customer_name,phone,airline,flight_number,trip_type,transport,
        ...
        priority,internal_note,archived_at,updated_at
      FROM lounge_bookings WHERE archived_at IS NULL
      UNION ALL
      SELECT id::bigint AS sort_id,(-id)::int AS id,reference,captain_name AS customer_name,passenger_phone AS phone,lounge_name AS airline,
        ...
        priority,internal_note,archived_at,updated_at
      FROM captain_lounge_orders WHERE archived_at IS NULL
  ...
  const companies=await db`
    ...
    SELECT n.name,
      ...
      (SELECT code FROM company_promo_codes p WHERE p.company_name=n.name AND p.active=TRUE ORDER BY created_at DESC LIMIT 1) AS promo_code
    FROM names n ORDER BY n.name`;
  const promoActivity=await db`
    SELECT reference,customer_name,promo_code,discount_iqd,created_at
    FROM lounge_bookings WHERE promo_code IS NOT NULL AND TRIM(promo_code)<>'' ORDER BY created_at DESC LIMIT 10`;
  return { stats:stats[0],captains:captainStats[0],captainList,promos:promoStats[0],recent,companies,promoActivity };
}

export async function listAdminBookings(limit=250){
  await prepare();
  const db=sql();
  return await db`
    SELECT * FROM (
...
export async function setBookingStatus(id:number,status:string){
  return await updateOrderOps({id,status});
}

export async function updateAdminBooking(input:{id:number;status?:string;priority?:"normal"|"important"|"urgent";internalNote?:string;archived?:boolean}){
  return await updateOrderOps(input);
}
